defmodule FiletransferWeb.UploadController do
  ...
  def init_multipart(conn, %{"id" => transfer_id}) do
    ...
    with {:ok, transfer} <- get_user_transfer(user.id, transfer_id),
      ...
      json(conn, %{
        status: "success",
        data: %{upload_id: upload_id, key: key, transfer_id: transfer.id}
      })
    end
  end
  ...
  def upload_chunk(conn, %{"id" => transfer_id} = params) do
    ...
    with {:ok, transfer} <- get_user_transfer(user.id, transfer_id),
      ...
      {:ok, _updated} =
        Transfers.update_chunk_progress(transfer.id, part_number - 1, bytes_uploaded)

      TransferChannel.broadcast_progress(transfer.id, part_number - 1, bytes_uploaded)

      json(conn, %{
        status: "success",
        data: %{part_number: part_number, etag: etag, bytes_uploaded: bytes_uploaded}
      })
    end
  end
  ...
  def complete_upload(conn, %{"id" => transfer_id, "parts" => parts}) do
    ...
    with {:ok, transfer} <- get_user_transfer(user.id, transfer_id),
         ...
         {:ok, key} <-
           Storage.complete_multipart_upload(transfer.storage_path, upload_id, parts_list) do
      {:ok, _completed} =
        Transfers.update_transfer(transfer, %{
          status: "completed",
      ...
      json(conn, %{
        status: "success",
        data: %{transfer_id: transfer.id, storage_path: key, status: "completed"}
      })
    end
  end
  ...
  def presigned_url(conn, %{"id" => transfer_id} = params) do
    ...
    with {:ok, transfer} <- get_user_transfer(user.id, transfer_id) do
      key =
        transfer.storage_path || Storage.generate_key(user.id, transfer.id, transfer.file_name)

      if is_nil(transfer.storage_path),
        do: Transfers.update_transfer(transfer, %{storage_path: key})

      {:ok, url} = Storage.presigned_upload_url(key, expires_in)

      json(conn, %{
        status: "success",
        data: %{url: url, key: key, expires_in: expires_in, method: "PUT"}
      })
    end
  end
