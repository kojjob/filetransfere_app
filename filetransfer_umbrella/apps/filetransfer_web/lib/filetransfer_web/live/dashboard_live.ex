defmodule FiletransferWeb.DashboardLive do
  ...
  defp sample_notifications do
    [
      %{id: 1, type: :upload, title: "Upload Complete", message: "report.pdf uploaded successfully", time: "2 min ago", read: false},
      %{id: 2, type: :share, title: "New Download", message: "Someone downloaded your shared file", time: "1 hour ago", read: false},
      %{id: 3, type: :system, title: "Storage Alert", message: "You're using 75% of your storage", time: "Yesterday", read: true}
    ]
  end
  ...
  defp calculate_stats(user) do
    ...
    shares = case Sharing.list_user_share_links(user.id) do
      {:ok, list} -> list
      _ -> []
    end

    today = Date.utc_today()
    uploads_today = Enum.count(transfers, fn t ->
  ...
  defp default_stats do
    %{uploads_today: 0, total_uploads: 0, active_shares: 0, downloads_total: 0,
      storage_used: 0, storage_limit: 10_000_000_000, completed_transfers: 0, pending_transfers: 0}
  end
  ...
  defp calculate_storage_breakdown(user) do
    transfers = case Transfers.list_transfers(user.id) do
      {:ok, list} -> list
      _ -> []
    end
    ...
    |> Enum.map(fn {category, files} ->
      %{category: category, count: length(files),
  ...
  defp get_file_category(transfer) do
    ...
    cond do
      ext in ~w(.jpg .jpeg .png .gif .webp .svg .bmp .ico) -> :images
      ext in ~w(.mp4 .mov .avi .mkv .webm .flv .wmv) -> :videos
      ext in ~w(.mp3 .wav .flac .aac .ogg .wma .m4a) -> :audio
      ext in ~w(.pdf .doc .docx .xls .xlsx .ppt .pptx .txt .rtf .odt) -> :documents
      ext in ~w(.zip .rar .7z .tar .gz .bz2) -> :archives
      ext in ~w(.js .ts .py .rb .ex .exs .go .rs .java .c .cpp .h .css .html .json .xml .yml .yaml) -> :code
      true -> :other
    end
  end
  ...
  defp get_recent_activity(user) do
    ...
    |> Enum.take(8)
    |> Enum.map(fn t ->
      %{id: t.id, type: :upload, file_name: t.file_name, file_size: t.file_size,
        status: t.status, timestamp: t.inserted_at, icon: file_icon(t.file_name)}
    end)
  end
  ...
  def handle_event("toggle_user_menu", _, socket) do
    {:noreply, socket |> assign(:show_user_menu, !socket.assigns.show_user_menu) |> assign(:show_notifications, false)}
  end

  @impl true
  def handle_event("toggle_notifications", _, socket) do
    {:noreply, socket |> assign(:show_notifications, !socket.assigns.show_notifications) |> assign(:show_user_menu, false)}
  end
  ...
  def handle_event("select_file", %{"id" => id}, socket) do
    ...
    file_shares = if transfer && socket.assigns.current_user do
  ...
  def handle_event("delete_transfer", %{"id" => id}, socket) do
    case Transfers.delete_transfer(id) do
      {:ok, _} ->
        {:noreply, socket |> put_flash(:info, "File deleted") |> assign(:selected_file, nil) |> load_dashboard_data()}
      {:error, _} ->
  ...
  def handle_event("create_share_link", %{"id" => id}, socket) do
    case Sharing.create_share_link(socket.assigns.current_user.id, id) do
      {:ok, _share} ->
        ...
        {:noreply, socket |> put_flash(:info, "Share link created!") |> assign(:file_shares, file_shares) |> load_dashboard_data()}
      {:error, _} ->
  ...
  defp tc(theme) do
    if theme == "light" do
      %{
        ...
        text: "text-gray-900", muted: "text-gray-500", subtle: "text-gray-400",
        input: "bg-gray-100 border-gray-200", hover: "hover:bg-gray-100", border: "border-gray-200",
        header: "bg-white/80", kbd: "bg-gray-200", progress: "bg-gray-200", row: "bg-gray-50",
        divider: "bg-gray-200", dropdown: "bg-white border-gray-200 shadow-lg"
      }
    else
      %{
        bg: "bg-slate-900", sidebar: "bg-slate-800/95 border-slate-700/50", card: "bg-slate-800/50 border-slate-700/50",
        text: "text-white", muted: "text-slate-400", subtle: "text-slate-500",
        input: "bg-slate-800/50 border-slate-700/50", hover: "hover:bg-slate-700/50", border: "border-slate-700/50",
        header: "bg-slate-900/80", kbd: "bg-slate-700/50", progress: "bg-slate-700", row: "bg-slate-700/30",
        divider: "bg-slate-700", dropdown: "bg-slate-800 border-slate-700 shadow-xl"
      }
    end
  end
  ...
  def render(assigns) do
    ...
    ~H"""
    <div class={"min-h-screen flex transition-colors duration-300 #{@t.bg}"} phx-click="close_dropdowns">
      <!-- Sidebar -->
      <aside class={["fixed inset-y-0 left-0 z-50 flex flex-col backdrop-blur-xl border-r transition-all duration-300", @t.sidebar, @sidebar_collapsed && "w-20", !@sidebar_collapsed && "w-72"]}>
        <div class={"h-16 flex items-center justify-between px-4 border-b #{@t.border}"}>
          <div class="flex items-center gap-3">
            ...
            <span class={["font-bold text-xl transition-opacity", @t.text, @sidebar_collapsed && "opacity-0 w-0 overflow-hidden"]}>ZipShare</span>
          </div>
          <button phx-click="toggle_sidebar" class={"p-2 rounded-lg #{@t.hover} #{@t.muted}"}>
            <.icon name={if @sidebar_collapsed, do: "hero-chevron-right", else: "hero-chevron-left"} class="w-5 h-5" />
          </button>
        </div>

        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          <.nav_btn icon="hero-squares-2x2" label="Dashboard" tab={:dashboard} current={@current_tab} collapsed={@sidebar_collapsed} theme={@theme} />
          <.nav_btn icon="hero-cloud-arrow-up" label="Upload" tab={:upload} current={@current_tab} collapsed={@sidebar_collapsed} theme={@theme} />
          <.nav_btn icon="hero-folder" label="My Files" tab={:files} current={@current_tab} collapsed={@sidebar_collapsed} theme={@theme} badge={@stats.total_uploads} />
          <.nav_btn icon="hero-link" label="Shared Links" tab={:shares} current={@current_tab} collapsed={@sidebar_collapsed} theme={@theme} badge={@stats.active_shares} />
          <.nav_btn icon="hero-clock" label="Recent" tab={:recent} current={@current_tab} collapsed={@sidebar_collapsed} theme={@theme} />
          <.nav_btn icon="hero-star" label="Favorites" tab={:favorites} current={@current_tab} collapsed={@sidebar_collapsed} theme={@theme} />
          <.nav_btn icon="hero-trash" label="Deleted" tab={:deleted} current={@current_tab} collapsed={@sidebar_collapsed} theme={@theme} />
          <div class={"pt-4 mt-4 border-t #{@t.border}"}>
            <.nav_btn icon="hero-cog-6-tooth" label="Settings" tab={:settings} current={@current_tab} collapsed={@sidebar_collapsed} theme={@theme} />
          </div>
        </nav>

        <div class={["px-4 py-4 border-t", @t.border, @sidebar_collapsed && "px-2"]}>
          <div class={["mb-2 flex items-center justify-between", @sidebar_collapsed && "justify-center"]}>
            <span class={["text-sm", @t.muted, @sidebar_collapsed && "hidden"]}>Storage</span>
            <span class={["text-sm font-medium", @t.text, @sidebar_collapsed && "hidden"]}><%= fmt_storage(@stats.storage_used) %> / <%= fmt_storage(@stats.storage_limit) %></span>
          </div>
          <div class={"h-2 rounded-full overflow-hidden #{@t.progress}"}>
            <div class="h-full bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full" style={"width: #{min(100, (@stats.storage_used / max(@stats.storage_limit, 1)) * 100)}%"} />
          </div>
          <button class={["mt-3 w-full py-2 px-3 rounded-lg bg-gradient-to-r from-cyan-500 to-blue-500 text-white text-sm font-medium hover:from-cyan-600 hover:to-blue-600", @sidebar_collapsed && "px-2"]}>
      ...
      <!-- Main -->
      <main class={["flex-1 transition-all duration-300", @sidebar_collapsed && "ml-20", !@sidebar_collapsed && "ml-72", @selected_file && "mr-96"]}>
        <header class={["sticky top-0 z-40 h-16 backdrop-blur-xl border-b flex items-center justify-between px-6", @t.header, @t.border]}>
          <div class="flex items-center gap-4">
            <div class="relative">
              <.icon name="hero-magnifying-glass" class={"absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 #{@t.muted}"} />
              <input type="text" placeholder="Search files..." phx-change="search" name="query" value={@search_query} class={"w-80 pl-10 pr-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 #{@t.text} #{@t.input}"} />
              <kbd class={"absolute right-3 top-1/2 -translate-y-1/2 px-2 py-0.5 text-xs rounded #{@t.muted} #{@t.kbd}"}>⌘K</kbd>
            </div>
          </div>

          <div class="flex items-center gap-3">
            <!-- Theme Toggle -->
            <button phx-click="toggle_theme" class={"p-2 rounded-xl #{@t.hover} #{@t.muted}"} title={if @theme == "dark", do: "Light mode", else: "Dark mode"}>
              <.icon name={if @theme == "dark", do: "hero-sun", else: "hero-moon"} class="w-5 h-5" />
            </button>

            <!-- Upload Button -->
            <button phx-click="show_upload_modal" class="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-medium rounded-xl hover:from-cyan-600 hover:to-blue-600 shadow-lg shadow-cyan-500/25">
              <.icon name="hero-cloud-arrow-up" class="w-5 h-5" /> Upload
            </button>

            <!-- Notifications Dropdown -->
            <div class="relative" phx-click-away="close_dropdowns">
              <button phx-click="toggle_notifications" class={"p-2 rounded-xl #{@t.hover} #{@t.muted} relative"}>
                <.icon name="hero-bell" class="w-5 h-5" />
                <span :if={@unread_count > 0} class="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-xs rounded-full flex items-center justify-center font-medium">
                  <%= @unread_count %>
                </span>
              </button>

              <!-- Notifications Panel -->
              <div :if={@show_notifications} class={["absolute right-0 top-full mt-2 w-80 rounded-xl border overflow-hidden z-50", @t.dropdown]} phx-click="noop">
                <div class={"p-4 border-b flex items-center justify-between #{@t.border}"}>
                  <h3 class={"font-semibold #{@t.text}"}>Notifications</h3>
                  <div class="flex items-center gap-2">
                    <button :if={@unread_count > 0} phx-click="mark_all_read" class="text-xs text-cyan-500 hover:text-cyan-400">Mark all read</button>
                    <button :if={length(@notifications) > 0} phx-click="clear_notifications" class={"text-xs #{@t.muted} hover:text-red-400"}>Clear</button>
                  </div>
                </div>
                <div class="max-h-80 overflow-y-auto">
                  <%= for notification <- @notifications do %>
                    <div
                      ...
                      class={["p-4 cursor-pointer transition-colors #{@t.hover}", !notification.read && (if @theme == "light", do: "bg-cyan-50", else: "bg-cyan-500/10")]}
                    >
                      <div class="flex items-start gap-3">
                        ...
                        <div :if={!notification.read} class="w-2 h-2 bg-cyan-500 rounded-full flex-shrink-0 mt-2"></div>
                      </div>
                    </div>
                  <% end %>
                ...
                <div class={"p-3 border-t text-center #{@t.border}"}>
                  <button class="text-sm text-cyan-500 hover:text-cyan-400">View all notifications</button>
                </div>
              </div>
            </div>
            ...
            <!-- User Menu Dropdown -->
            <div class="relative" phx-click-away="close_dropdowns">
              <button phx-click="toggle_user_menu" class="flex items-center gap-2 p-1 rounded-xl hover:bg-slate-700/30 transition-colors">
                ...
                <.icon name="hero-chevron-down" class={"w-4 h-4 #{@t.muted} transition-transform #{@show_user_menu && "rotate-180"}"} />
              </button>

              <!-- User Menu Panel -->
              <div :if={@show_user_menu} class={["absolute right-0 top-full mt-2 w-64 rounded-xl border overflow-hidden z-50", @t.dropdown]} phx-click="noop">
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                      <p class={"font-medium truncate #{@t.text}"}>
                        <%= if @current_user, do: @current_user.name || @current_user.email, else: "Guest User" %>
                      </p>
                ...
                <div class="p-2">
                  <button phx-click="switch_tab" phx-value-tab="settings" class={"w-full flex items-center gap-3 px-3 py-2 rounded-lg #{@t.hover} #{@t.text}"}>
                ...
                <div class={"p-2 border-t #{@t.border}"}>
                  <button phx-click="logout" class="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-red-500/10 text-red-500">
        ...
        <div class="p-6">
          <%= case @current_tab do %>
            <% :dashboard -> %> <.dashboard_tab stats={@stats} transfers={@recent_transfers} shares={@active_shares} breakdown={@storage_breakdown} t={@t} theme={@theme} />
            <% :upload -> %> <.upload_tab uploads={@uploads} progress={@upload_progress} t={@t} theme={@theme} />
            <% :files -> %> <.files_tab transfers={@recent_transfers} selected={@selected_file} t={@t} theme={@theme} />
            <% :shares -> %> <.shares_tab shares={@active_shares} t={@t} theme={@theme} />
            <% :recent -> %> <.recent_tab activity={@recent_activity} t={@t} theme={@theme} />
            <% :settings -> %> <.settings_tab current_user={@current_user} t={@t} theme={@theme} />
            <% _ -> %> <.coming_soon tab={@current_tab} t={@t} />
          <% end %>
        </div>
      </main>

      <!-- Enhanced File Preview Panel -->
      <aside :if={@selected_file} class={["fixed inset-y-0 right-0 w-96 backdrop-blur-xl border-l z-50 overflow-y-auto", @t.sidebar]}>
        <.file_preview file={@selected_file} shares={@file_shares} t={@t} theme={@theme} />
      </aside>

      <.upload_modal :if={@show_upload_modal} uploads={@uploads} progress={@upload_progress} t={@t} theme={@theme} />
    </div>
    """
  end
  ...
  defp nav_btn(assigns) do
    active = if assigns.theme == "light", do: "bg-cyan-50 text-cyan-600", else: "bg-gradient-to-r from-cyan-500/20 to-blue-500/20 text-white"
    inactive = if assigns.theme == "light", do: "text-gray-600 hover:text-gray-900 hover:bg-gray-100", else: "text-slate-400 hover:text-white hover:bg-slate-700/50"
    assigns = assigns |> assign(:active, active) |> assign(:inactive, inactive)

    ~H"""
    <button phx-click="switch_tab" phx-value-tab={@tab} class={["w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all", @current == @tab && @active, @current != @tab && @inactive]}>
      <.icon name={@icon} class="w-5 h-5 flex-shrink-0" />
      <span class={["flex-1 text-left", @collapsed && "hidden"]}><%= @label %></span>
      <span :if={@badge && @badge > 0 && !@collapsed} class="px-2 py-0.5 text-xs bg-cyan-500/20 text-cyan-400 rounded-full"><%= @badge %></span>
    </button>
    """
  end
  ...
  defp dashboard_tab(assigns) do
    ...
    <div class="space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <.stat_card title="Uploads Today" value={@stats.uploads_today} icon="hero-cloud-arrow-up" color="cyan" trend="+12%" t={@t} theme={@theme} />
        <.stat_card title="Active Shares" value={@stats.active_shares} icon="hero-link" color="purple" t={@t} theme={@theme} />
        <.stat_card title="Downloads" value={@stats.downloads_total} icon="hero-arrow-down-tray" color="green" trend="+8%" t={@t} theme={@theme} />
        <.stat_card title="Storage Used" value={fmt_storage(@stats.storage_used)} icon="hero-server" color="orange" sub={"of #{fmt_storage(@stats.storage_limit)}"} t={@t} theme={@theme} />
      </div>
      ...
      <div class="grid lg:grid-cols-3 gap-6">
        <div class={["lg:col-span-2 rounded-2xl border overflow-hidden", @t.card]}>
          <div class={"px-6 py-4 border-b flex items-center justify-between #{@t.border}"}>
            ...
            <button phx-click="switch_tab" phx-value-tab="files" class="text-sm text-cyan-400 hover:text-cyan-300">See all</button>
          </div>
          <div class={"divide-y #{@t.border}"}>
            <%= for transfer <- Enum.take(@transfers, 5) do %>
              <div phx-click="select_file" phx-value-id={transfer.id} class={"px-6 py-4 flex items-center gap-4 cursor-pointer #{@t.hover}"}>
                <div class={"w-12 h-12 rounded-xl flex items-center justify-center #{file_bg(transfer.file_name)}"}><.icon name={file_icon(transfer.file_name)} class="w-6 h-6 text-white" /></div>
                <div class="flex-1 min-w-0">
            ...
            <div :if={Enum.empty?(@transfers)} class="px-6 py-12 text-center">
              ...
              <button phx-click="switch_tab" phx-value-tab="upload" class="mt-3 text-sm text-cyan-400 hover:text-cyan-300">Upload your first file</button>
            </div>
          </div>
        </div>

        <div class="space-y-6">
          <div class={"rounded-2xl border border-dashed p-6 text-center cursor-pointer hover:border-cyan-500/50 #{@t.card}"} phx-click="switch_tab" phx-value-tab="upload">
          ...
          <div class={["rounded-2xl border p-6", @t.card]}>
            ...
            <div class="space-y-3">
              ...
              <% end %>
              <div :if={Enum.empty?(@breakdown)} class="text-center py-4"><p class={"text-sm #{@t.muted}"}>No storage data</p></div>
            </div>
          </div>
        </div>
      </div>
  ...
  defp upload_tab(assigns) do
    ...
    <div class="max-w-4xl mx-auto space-y-6">
      ...
      <form phx-change="validate" phx-submit="start_upload">
        <div class={["border-2 border-dashed rounded-3xl p-12 text-center hover:border-cyan-500/50 transition-all", @t.card]} phx-drop-target={@uploads.files.ref}>
          ...
          <p class={"mt-6 text-sm #{@t.subtle}"}>Maximum file size: 10GB • All file types supported</p>
        </div>
          ...
          <div class="mt-8 space-y-3">
            <%= for entry <- @uploads.files.entries do %>
              <div class={["p-4 rounded-xl border flex items-center gap-4", @t.card]}>
                <div class={"w-12 h-12 rounded-xl flex items-center justify-center #{file_bg(entry.client_name)}"}><.icon name={file_icon(entry.client_name)} class="w-6 h-6 text-white" /></div>
                <div class="flex-1 min-w-0">
                  <p class={"font-medium truncate #{@t.text}"}><%= entry.client_name %></p>
                  <p class={"text-sm #{@t.muted}"}><%= fmt_size(entry.client_size) %></p>
                </div>
                <button type="button" phx-click="cancel_upload" phx-value-ref={entry.ref} class={"p-2 rounded-lg hover:bg-red-500/10 hover:text-red-400 #{@t.muted}"}>
                  <.icon name="hero-x-mark" class="w-5 h-5" />
                </button>
              </div>
            <% end %>
            <button type="submit" class="w-full mt-4 py-4 px-6 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-semibold rounded-xl shadow-lg shadow-cyan-500/25 flex items-center justify-center gap-2">
  ...
  defp settings_tab(assigns) do
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-start gap-6">
          ...
          <div class="flex-1 space-y-4">
            <div>
              ...
              <p class={"mt-1 #{@t.text}"}><%= if @current_user, do: @current_user.email, else: "Not logged in" %></p>
            </div>
            <div>
              <label class={"text-sm font-medium #{@t.muted}"}>Name</label>
              <p class={"mt-1 #{@t.text}"}><%= if @current_user && @current_user.name, do: @current_user.name, else: "Not set" %></p>
            </div>
            <button class="px-4 py-2 bg-cyan-500 text-white rounded-lg hover:bg-cyan-600">Edit Profile</button>
          </div>
        </div>
      </div>
      ...
      <div class={["rounded-2xl border p-6", @t.card]}>
        ...
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            ...
            <button phx-click="toggle_theme" class={["w-12 h-6 rounded-full transition-colors", @theme == "dark" && "bg-cyan-500", @theme == "light" && "bg-gray-300"]}>
              <div class={["w-5 h-5 rounded-full bg-white shadow transition-transform", @theme == "dark" && "translate-x-6", @theme == "light" && "translate-x-0.5"]}></div>
            </button>
          </div>
      ...
      <div class="rounded-2xl border border-red-500/30 p-6 bg-red-500/5">
        ...
        <p class={"mb-4 #{@t.muted}"}>Once you delete your account, there is no going back. Please be certain.</p>
        <button class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">Delete Account</button>
      </div>
    </div>
    """
  end
  ...
  defp stat_card(assigns) do
    colors = %{
      "cyan" => if(assigns.theme == "light", do: "from-cyan-50 to-cyan-50/50 border-cyan-200", else: "from-cyan-500/20 to-cyan-500/5 border-cyan-500/20"),
      "purple" => if(assigns.theme == "light", do: "from-purple-50 to-purple-50/50 border-purple-200", else: "from-purple-500/20 to-purple-500/5 border-purple-500/20"),
      "green" => if(assigns.theme == "light", do: "from-green-50 to-green-50/50 border-green-200", else: "from-green-500/20 to-green-500/5 border-green-500/20"),
      "orange" => if(assigns.theme == "light", do: "from-orange-50 to-orange-50/50 border-orange-200", else: "from-orange-500/20 to-orange-500/5 border-orange-500/20")
    }
    icons = %{"cyan" => "text-cyan-500", "purple" => "text-purple-500", "green" => "text-green-500", "orange" => "text-orange-500"}
    ...
    <div class={"p-5 rounded-2xl bg-gradient-to-br border #{@bg}"}>
      <div class="flex items-start justify-between">
        <div>
          ...
          <p :if={@trend} class="text-xs text-green-500 mt-1"><.icon name="hero-arrow-trending-up" class="w-3 h-3 inline" /> <%= @trend %></p>
        </div>
        <div class={["p-3 rounded-xl", if(@theme == "light", do: "bg-white shadow-sm", else: "bg-slate-800/50"), @ic]}><.icon name={@icon} class="w-6 h-6" /></div>
      </div>
    </div>
    """
  end
  ...
  defp files_tab(assigns) do
    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        <%= for tr <- @transfers do %>
          <div phx-click="select_file" phx-value-id={tr.id} class={["p-4 rounded-2xl border cursor-pointer hover:border-cyan-500/50 transition-all", @t.card, @selected && @selected.id == tr.id && "ring-2 ring-cyan-500"]}>
            <div class={"w-full aspect-square rounded-xl flex items-center justify-center mb-3 #{file_bg(tr.file_name)}"}><.icon name={file_icon(tr.file_name)} class="w-10 h-10 text-white" /></div>
      ...
      <div :if={Enum.empty?(@transfers)} class="text-center py-20">
        ...
        <button phx-click="switch_tab" phx-value-tab="upload" class="mt-4 text-cyan-500 hover:text-cyan-400">Upload your first file →</button>
      </div>
    </div>
    """
  end
        ...
        <table class="w-full">
          <thead class={@t.row}>
            <tr>
              ...
              <th class={"px-6 py-3 text-left text-xs font-medium uppercase #{@t.muted}"}>Downloads</th>
          ...
          <tbody class={"divide-y #{@t.border}"}>
            <%= for s <- @shares do %>
              <tr class={@t.hover}>
                <td class="px-6 py-4">
                  <div class="flex items-center gap-3">
                    <div class={"w-10 h-10 rounded-lg flex items-center justify-center #{file_bg(s.transfer && s.transfer.file_name)}"}><.icon name={file_icon(s.transfer && s.transfer.file_name)} class="w-5 h-5 text-white" /></div>
                    <span class={"truncate max-w-[200px] #{@t.text}"}><%= if s.transfer, do: s.transfer.file_name, else: "Unknown" %></span>
                  </div>
                </td>
                <td class="px-6 py-4"><code class="text-sm text-cyan-500 bg-cyan-500/10 px-2 py-1 rounded">/s/<%= s.token %></code></td>
                <td class={"px-6 py-4 #{@t.text}"}><%= s.download_count || 0 %></td>
                <td class={"px-6 py-4 #{@t.muted}"}><%= if s.expires_at, do: fmt_rel(s.expires_at), else: "Never" %></td>
              </tr>
            <% end %>
          </tbody>
        </table>
      ...
      <div class={["rounded-2xl border divide-y", @t.card, @t.border]}>
        <%= for a <- @activity do %>
          <div class={"px-6 py-4 flex items-center gap-4 #{@t.hover}"}>
            <div class={"w-10 h-10 rounded-xl flex items-center justify-center #{file_bg(a.file_name)}"}><.icon name={a.icon} class="w-5 h-5 text-white" /></div>
            <div class="flex-1">
  ...
  defp file_preview(assigns) do
    ext = Path.extname(assigns.file.file_name || "") |> String.trim_leading(".") |> String.upcase()
    mime = get_mime_type(assigns.file.file_name)
    category = get_file_category(assigns.file)
    total_downloads = Enum.reduce(assigns.shares, 0, fn s, acc -> acc + (s.download_count || 0) end)
    assigns = assigns |> assign(:ext, ext) |> assign(:mime, mime) |> assign(:category, category) |> assign(:total_downloads, total_downloads)

    ~H"""
    <div class="h-full flex flex-col">
      <div class={"p-4 border-b flex items-center justify-between #{@t.border}"}>
        <h3 class={"text-lg font-semibold #{@t.text}"}>File Details</h3>
        <button phx-click="close_preview" class={"p-2 rounded-lg #{@t.hover} #{@t.muted}"}><.icon name="hero-x-mark" class="w-5 h-5" /></button>
      </div>

      <div class="flex-1 overflow-y-auto">
        <div class="p-6">
          <div class={"w-full aspect-video rounded-2xl flex items-center justify-center mb-4 #{file_bg(@file.file_name)}"}><.icon name={file_icon(@file.file_name)} class="w-20 h-20 text-white" /></div>
        ...
        <div class="p-6 space-y-6">
          ...
          <div>
            <h5 class={"text-xs font-medium uppercase tracking-wider mb-3 #{@t.subtle}"}>Properties</h5>
            <div class={["rounded-xl divide-y", @t.row, @t.border]}>
              <.property_row label="Type" value={@ext || "Unknown"} icon="hero-document" t={@t} />
              <.property_row label="Category" value={to_string(@category) |> String.capitalize()} icon="hero-folder" t={@t} />
              ...
              <.property_row label="Uploaded" value={fmt_datetime(@file.inserted_at)} icon="hero-calendar" t={@t} />
            </div>
          </div>
      ...
      <div class={"p-4 border-t space-y-2 #{@t.border}"}>
        <button phx-click="create_share_link" phx-value-id={@file.id} class="w-full py-3 px-4 bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-medium rounded-xl hover:from-cyan-600 hover:to-blue-600 flex items-center justify-center gap-2">
          <.icon name="hero-link" class="w-5 h-5" /> Create Share Link
        </button>
        <div class="grid grid-cols-2 gap-2">
          <button class={["py-2.5 px-4 font-medium rounded-xl flex items-center justify-center gap-2", if(@theme == "light", do: "bg-gray-100 text-gray-700 hover:bg-gray-200", else: "bg-slate-700/50 text-white hover:bg-slate-700")]}>
        ...
        <button phx-click="delete_transfer" phx-value-id={@file.id} data-confirm="Delete this file?" class="w-full py-2.5 px-4 bg-red-500/10 text-red-500 font-medium rounded-xl hover:bg-red-500/20 flex items-center justify-center gap-2">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class={["relative w-full max-w-2xl mx-4 rounded-2xl border shadow-2xl", @t.card]}>
        <div class={"p-6 border-b flex items-center justify-between #{@t.border}"}>
          ...
          <button phx-click="close_upload_modal" class={"p-2 rounded-lg #{@t.hover} #{@t.muted}"}><.icon name="hero-x-mark" class="w-5 h-5" /></button>
        </div>
        <div class="p-6">
          <form phx-change="validate" phx-submit="start_upload">
            <div class={"border-2 border-dashed rounded-2xl p-8 text-center hover:border-cyan-500 #{@t.border}"} phx-drop-target={@uploads.files.ref}>
              <div class="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-cyan-500/20 to-blue-500/20 flex items-center justify-center"><.icon name="hero-cloud-arrow-up" class="w-8 h-8 text-cyan-400" /></div>
              <p class={"font-medium mb-1 #{@t.text}"}>Drag & drop files here</p>
              <p class={"text-sm mb-4 #{@t.muted}"}>or click to browse</p>
              <label class={["inline-flex items-center gap-2 px-4 py-2 rounded-xl cursor-pointer", if(@theme == "light", do: "bg-gray-100 hover:bg-gray-200 text-gray-700", else: "bg-slate-700/50 hover:bg-slate-700 text-white")]}>
                ...
                <%= for e <- @uploads.files.entries do %>
                  <div class={["p-4 rounded-xl", @t.row]}>
                    <div class="flex items-center justify-between mb-2">
                      <div class="flex items-center gap-3">
                        <div class={"w-10 h-10 rounded-lg flex items-center justify-center #{file_bg(e.client_name)}"}><.icon name={file_icon(e.client_name)} class="w-5 h-5 text-white" /></div>
                        <div><p class={"font-medium text-sm truncate max-w-[300px] #{@t.text}"}><%= e.client_name %></p><p class={"text-xs #{@t.muted}"}><%= fmt_size(e.client_size) %></p></div>
                      </div>
                      <button type="button" phx-click="cancel_upload" phx-value-ref={e.ref} class={"hover:text-red-400 #{@t.muted}"}><.icon name="hero-x-mark" class="w-5 h-5" /></button>
                    </div>
                    <div class={"h-1.5 rounded-full overflow-hidden #{@t.progress}"}><div class="h-full bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full" style={"width: #{Map.get(@progress, e.ref, 0)}%"} /></div>
                  </div>
                <% end %>
              </div>
              <button type="submit" class="mt-6 w-full py-3 px-4 bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-medium rounded-xl hover:from-cyan-600 hover:to-blue-600">Upload <%= length(@uploads.files.entries) %> files</button>
            <% end %>
          </form>
        </div>
      </div>
